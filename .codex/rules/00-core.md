...
| 토글 | 동작 |
|------|------|
| `/plan` | 코드 변경/실행 금지, docs 저장은 허용 |
| `/code` | **승인 게이트 1단계** - 변경 가능 모드 진입 (단독으로는 최종 승인 아님) |
| `@path` | 파일 찾기/네비게이션 |
| `스킬 OFF/ON` | 스킬 기능 kill-switch |

**적용 범위**:
- `/code` 게이트는 **소스코드/환경설정 변경**에만 적용.
- **문서(docs)만 변경**하는 경우 `/code` 없이 진행 가능(단, 사용자가 Plan-only를 명시한 경우 제외).
...
```
User: "로그인 코드 수정해줘"
AI: [1단계 미통과] 수정 계획을 제안합니다. 진행하려면 `/code`를 입력하세요.

User: "/code"
AI: [1단계 통과] 변경 가능 모드 진입. 타겟 repo를 지정해주세요.
