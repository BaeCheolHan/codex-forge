...
### 2. 완전성 (Completeness)
- [ ] 설치/적용/진단/업데이트 절차 완결
- [ ] 모든 시나리오(S0-S3, Hotfix) 커버
- [ ] 에러 케이스 및 폴백 명시

### 3. 중복 제거 (DRY)
- [ ] 동일 규칙이 여러 파일에 분산되지 않음
- [ ] 정본(00-core)에서 참조하는 구조
...
### 주요 변경사항
1. **버전 표기 정합성 정리**: CHANGELOG/SELF_REVIEW/RELEASE_CHECKLIST/system-prompt 업데이트
2. **VERSIONING 갱신**: 실제 파일 기준으로 버전 표기 위치 재정리
3. **릴리스 체크리스트 업데이트**: v2.3.3 예시/경로 정합화
